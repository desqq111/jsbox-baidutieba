/*
更新时间：2018年9月8号
更新内容：修复无法签到问题。支持多账号签到。增加添加BDUSS功能，添加管理账号，切换账号
*/
...
function getlb() {
  ...
  $http.request({
    ...
    handler: function(resp) {
      $ui.loading(false)
      var text=resp.data
      var shu = text.data.like_forum
      var tbs = text.data.tbs
      $cache.set("tbs", tbs)
      var data = []
      for (i in shu) {
        var story=shu[i]
        var zt = ""
        if (story.is_sign==1) { zt = "已签到" } else { zt = "未签" };
        data.push({
          mc: {
            text: story.forum_name
          },
          zt: {
            text: zt
          },
          dj: {
            text: story.user_level
          }
